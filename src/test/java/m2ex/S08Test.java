...
class S08Test {
	...
	@Test
	void BinaryPlainSum() {
		String sum = S08.binarySum("101", "010");
		assertThat(sum, is("111"));
	}

	@Test
	void BinaryMoreDigits() {
		String sum = S08.binarySum("111", "010");
		assertThat(sum, is("1001"));
	}

	@Test
	void BinaryFiveDigits() {
		String sum = S08.binarySum("11011", "10010");
		assertThat(sum, is("101101"));
	}

	@Test
	void BinaryAllOneFiveDigits() {
		String sum = S08.binarySum("11111", "11111");
	...
	void BinaryAllOneDigits() {
		String sum = S08.binarySum("111", "111");
		assertThat(sum, is("1110"));
	}

	@Test
	void BinaryMOneDigits() {
		String sum = S08.binarySum("1111", "111");
		assertThat(sum, is("11110"));
	}

	@Test
	void PlainSingle() {
		int sing = S08.getSingle(new int[] { 1, 2, 1, 2, 3, 4, 5, 4, 5 });
	...
	void PlainNotSingle() {
		assertThrows(UnsupportedOperationException.class, () -> {
			S08.getSingle(new int[] { 1, 2, 1, 2, 2, 4, 5, 4, 5 });
		});
	}

	@Test
	void PlainUnique() {
		boolean uni = S08.hasOnlyUnique("abe");
		assertThat(uni, is(true));
	}

	@Test
	void PlainNotUnique() {
		boolean uni = S08.hasOnlyUnique("aba");
		assertThat(uni, is(false));
	}

	@Test
	void PlainAnagram() {
		boolean ana = S08.isAnagram("abba", "baab");
		assertThat(ana, is(true));
	}

	@Test
	void PlainNotAnagram() {
		boolean ana = S08.isAnagram("abba", "bab");
		assertThat(ana, is(false));
	}

	@Test
	void NotAnagram() {
		boolean ana = S08.isAnagram("abba", "sbba");
		assertThat(ana, is(false));
	}

	@Test
	void PlainDuplicates() {
		int[] dup = S08.duplicates(new int[] { 1, 2, 1, 2, 3, 4, 5, 4, 5 });
